...
public class OthelloController extends EventPublisher implements EventListener {
	...
	public OthelloController(OthelloModel model, int role) {
		this.model = model;
		this.role = role;
		pieces = new OthelloPiece[model.getSize()][model.getSize()];
	}
	...
	public void doMove(int x, int y) throws BoardIndexOutOfBoundsException {
		if (!model.inBounds(x, y))
		...
		// Only publish the GameMoveEvent if it was your turn
		if (model.getTurn() == role)
			publishEvent(new GameMoveEvent(x, y));
	...
	public void handleEvent(Event event) {
		...
		case GameMoveEvent.type:
			GameMoveEvent gME = (GameMoveEvent) event;
	...
	public void setStates() {
		for (int i = 0; i < getPieces().length; i++)
			for (int j = 0; j < getPieces().length; j++)
				getPieces()[i][j].setState(model.getState(i, j));
	...
	private void clearGame() throws BoardIndexOutOfBoundsException {
		// Make pieces
		for (int i = 0; i < getPieces().length; i++)
			for (int j = 0; j < getPieces().length; j++) {
				model.setState(i, j, 0);
				getPieces()[i][j].setState(0);
			}
		EventBroker.getEventBroker().removeEventListener(this);
	}
